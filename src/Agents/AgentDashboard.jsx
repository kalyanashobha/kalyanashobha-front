      ...
      <div className={`app-input-wrap ${Icon ? 'app-with-icon' : ''}`} ref={wrapperRef} style={{ position: 'relative', zIndex: isOpen ? 100 : 1 }}>
        ...
        <div className="app-input-inner" style={{ position: 'relative' }}>
          ...
          <input 
            ...
            disabled={disabled}
            style={{ backgroundColor: disabled ? '#f1f5f9' : 'transparent', cursor: disabled ? 'not-allowed' : 'text' }}
          />
...
const AgentDashboard = () => {
  ...
  const [regData, setRegData] = useState({
    ...
    maritalStatus: '', height: '', diet: '', 
    community: '', caste: '', subCommunity: '', gothra: '',
    country: '', state: '', city: '', residentsIn: 'Own',
    highestQualification: '', collegeName: '', workType: '', 
    jobRole: '', companyName: '', annualIncome: '', nri: 'No',
  ...
  const fetchDynamicMasterData = async () => {
      // NOTE: Removed 'State' and 'City' from the initial load so they fetch dynamically later.
  ...
  // --- UPDATED DYNAMIC REGISTRATION CHANGE LOGIC ---
  const handleRegChange = async (e) => {
    ...
    if (name === 'community') {
      ...
      // Clear country, state and city
      setRegData({ ...regData, country: value, state: '', city: '' });
      setAvailableStates([]);
      setAvailableCities([]);
      
      // Fetch new states for the selected country
      if (value) {
        try {
      ...
      // Fetch new cities for the selected state
      if (value) {
        try {
      ...
      // Handle all other normal inputs
    ...
    switch(step) {
      ...
      case 3:
        ...
        if (regData.dob && regData.gender) {
            ...
            if (regData.gender === 'Male' && age < 21) {
                toast.error("Men must be at least 21 years old.");
                return false;
            } else if (regData.gender === 'Female' && age < 18) {
                toast.error("Women must be at least 18 years old.");
                return false;
            }
        }
        return true;
      case 4:
        if (!regData.community || !regData.caste || !regData.gothra) {
           toast.error("Please fill in all required socio-cultural details (including Gothra)"); return false;
        }
        return true;
      case 5:
                    ...
                    <div className="app-step-content app-slide-fwd">
                      ...
                      <div className="app-form-grid mt-4">
                        ...
                        <AgentSelect 
                          ...
                          value={regData.maritalStatus} onChange={handleRegChange} 
                          options={dynamicOptions.MaritalStatus} required={true} icon={Heart}
                        />
                      </div>
                    </div>
                  )}
                    ...
                    <div className="app-step-content app-slide-fwd">
                      ...
                      <div className="app-input-wrap app-with-icon mt-4">
                        ...
                        <div className="app-input-inner">
                          ...
                          <input type="text" name="gothra" value={regData.gothra} onChange={handleRegChange} placeholder="Enter Gothra" required />
                        </div>
                      </div>
                    </div>
                  )}
                  ...
                  {regStep === 5 && (
                    <div className="app-step-content app-slide-fwd">
                      ...
                      <div className="app-form-grid">
                          ...
                          />
                          <AgentSelect 
                            label="Work Sector" name="workType" 
                            value={regData.workType} onChange={handleRegChange} 
                            options={dynamicOptions.Sector} required={false} icon={Building2}
                          />
                      </div>
                      ...
                      <div className="app-form-grid mt-4">
                        ...
                        />
                        <AgentSelect 
                          label="NRI Status" name="nri" 
                          value={regData.nri} onChange={handleRegChange} 
                          options={['Yes', 'No']} required={true} icon={Globe}
                        />
                      </div>
                    </div>
                  )}
                  ...
                  {regStep === 6 && (
                    <div className="app-step-content app-slide-fwd">
                      ...
                      <div className="app-input-wrap app-with-icon mt-4">
                        ...
                        <div className="app-input-inner">
                          <Building2 size={18} className="app-field-icon" />
                          <select name="residentsIn" value={regData.residentsIn} onChange={handleRegChange} style={{border: 'none', background: 'transparent', width: '100%', outline: 'none'}}>
                      ...
                      {/* --- NESTED COUNTRY -> STATE -> CITY LOGIC ADDED HERE --- */}
                      <div className="mt-4">
                        <AgentComboInput 
                      ...
                      <div className="app-form-grid mt-4">
                        <div onClick={() => !regData.country && toast.info("Please select a Country first")}>
                          <AgentComboInput 
                            label="State" name="state" 
                            value={regData.state} onChange={handleRegChange} 
                            options={availableStates} required={true} icon={Map}
                            disabled={!regData.country}
                          />
                        </div>
                        <div onClick={() => regData.country && !regData.state && toast.info("Please select a State first")}>
                          <AgentComboInput 
                            label="City" name="city" 
                            value={regData.city} onChange={handleRegChange} 
                            options={availableCities} required={true} icon={MapPin}
