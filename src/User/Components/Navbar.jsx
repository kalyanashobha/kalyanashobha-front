        ...
        <nav className="main-nav">
          <ul className="main-nav-list">
            ...
            <li className="main-nav-item ks-dropdown-container">
              {/* Added /about and /testimonials to the active check */}
              <button className={`main-nav-link ks-dropdown-trigger ${['/terms', '/refund', '/faq', '/help', '/preference', '/about', '/testimonials'].includes(location.pathname) ? 'active' : ''}`}>
                More <ChevronDown size={14} strokeWidth={2.5} className="dropdown-arrow" />
              </button>
              
              <div className="ks-dropdown-menu">
                <Link to="/about" className="ks-dropdown-item">
                  <span className="nav-icon-box color-teal"><Users size={16} strokeWidth={2} /></span> 
                  <div className="dropdown-text">
                    <span className="dropdown-title">About Us</span>
                    <span className="dropdown-desc">Discover our story</span>
                  </div>
                </Link>
                <Link to="/testimonials" className="ks-dropdown-item">
                  <span className="nav-icon-box color-pink"><Star size={16} strokeWidth={2} /></span> 
                  <div className="dropdown-text">
                    <span className="dropdown-title">Testimonials</span>
                    <span className="dropdown-desc">Hear from our users</span>
                  </div>
                </Link>
                ...
                {/* Conditionally rendered Help Center and Preferences */}
                {isLoggedIn && (
                  <>
                    <Link to="/preference" className="ks-dropdown-item">
                      <span className="nav-icon-box color-purple"><Sliders size={16} strokeWidth={2} /></span> 
                      <div className="dropdown-text">
                        <span className="dropdown-title">Partner Preferences</span>
                        <span className="dropdown-desc">Set your ideal match criteria</span>
                      </div>
                    </Link>
          ...
          <ul className="mobile-menu-list">
            ...
            {isLoggedIn && (
              <>
                ...
                <li style={{ '--i': 4 }}>
                  <Link to="/preference" onClick={closeMenu} className={isActive('/preference') ? 'mobile-active' : ''}>
                    <span className="nav-icon-box color-purple"><Sliders size={16} strokeWidth={2.5} /></span> Partner Preferences
                  </Link>
                </li>
                <li style={{ '--i': 5 }}>
            ...
            <li style={{ '--i': 7 }}>
              <Link to="/about" onClick={closeMenu} className={`mobile-menu-link-small ${isActive('/about') ? 'mobile-active' : ''}`}>
                <span className="nav-icon-box color-teal"><Users size={14} strokeWidth={2.5} /></span> About Us
              </Link>
            </li>
            <li style={{ '--i': 8 }}>
              <Link to="/testimonials" onClick={closeMenu} className={`mobile-menu-link-small ${isActive('/testimonials') ? 'mobile-active' : ''}`}>
                <span className="nav-icon-box color-pink"><Star size={14} strokeWidth={2.5} /></span> Testimonials
              </Link>
            </li>
            <li style={{ '--i': 9 }}>
